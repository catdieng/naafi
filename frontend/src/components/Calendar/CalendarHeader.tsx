...
const CalendarHeader = () => {
  const { currentDate, goToday, locale, messages, next, prev, view, setView, viewRange } = useCalendar();

  const formattedMonth = format(currentDate, "MMMM yyyy", { locale: locale ?? undefined });
        ...
        <Flex direction="column">
          ...
          {view === "week" && viewRange && (
            <Text fontSize="xs" color="gray.500">
              {format(viewRange.start, "MMM d", { locale: locale ?? undefined })} - {format(viewRange.end, "MMM d", { locale: locale ?? undefined })}
            </Text>
          )}
        </Flex>
